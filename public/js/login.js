...
$(document).ready(function(){
  ...
  socket.on('validar_disponibilidad',function(datos){
    if(datos.estado=='1'){
      $('.wrap').html('').css('display','none');
    }else{
      $('.wrap').html(datos.msj).css('display','block');
    }
  });
  $("#btn-registrarme").on('click',function(){
    $("#frm-registro").removeClass("display-none");
    $("#frm-login").addClass("display-none");
  });
  $("#btn-cancelar-registro").on('click',function(){
    $("#frm-login").removeClass("display-none");
    $("#frm-registro").addClass("display-none");
  });
  $("#btn-volver").on('click',function(){window.location.replace('/')});
  $("#txt-usuario-registro").on('blur',function(){
    var usuario = $(this).val();
    if(usuario!='' && $.trim(usuario.length) > 4){
      socket.emit('validar_disponibilidad',usuario);
    }
  });
